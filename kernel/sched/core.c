...
long sched_setaffinity(pid_t pid, const struct cpumask *in_mask)
{
    ...
    cpumask_t new_cpumask;
    cpumask_setall(&new_cpumask);
    cpumask_setall(&old_cpumask);
#endif
	rcu_read_lock();
...
#ifdef JOURNEY_DEBUG_ENHANCED
    sched_getaffinity(pid,&old_cpumask);
#endif
...
#ifdef JOURNEY_DEBUG_ENHANCED
	if(retval == 0) {
		sched_getaffinity(pid,&new_cpumask);
		if(!cpumask_equal(&old_cpumask,&new_cpumask)) {
			printk("sched_setaffinity to pid %d target:%d-%d caller:%d-%d cpu from %*pb(%*pbl) to %*pb(%*pbl)",
				pid,p->tgid,p->pid,current->tgid,current->pid,
				cpumask_pr_args(&old_cpumask),
				cpumask_pr_args(&old_cpumask),
				cpumask_pr_args(&new_cpumask),
				cpumask_pr_args(&new_cpumask)
			);
		}
	}
