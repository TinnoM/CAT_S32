...
static int ff_ctl_free_driver(void)
{
    ...
    if(1) {//add by lijian for disable other fp ic irq wake fun.
...
static int __init ff_ctl_driver_init(void)
{
    int err = 0;
    FF_LOGV("'%s' enter.", __func__);
#if CMDLINE_FP_INFO //LIUJ201909181940ADDO
    if(0 == is_use_focalfp()){//lijian add for check fp info
   	FF_LOGI("ssss---not use focal fp\n");
   	return (-ENODEV);
    }
#endif
	...
	#endif
	{
	    FF_LOGI("FocalTech fingerprint chip init");
