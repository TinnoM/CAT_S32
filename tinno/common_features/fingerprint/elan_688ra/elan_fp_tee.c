...
#define FTM_NORMAL_STATUS 0
#define FTM_START_STATUS 2
#define FTM_END_STATUS  4

#define TRUE_IRQ_NOTIFICATION  1
...
static int factory_status = FTM_NORMAL_STATUS;
...
static long efsa120s_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	switch(cmd)
	{
		...
		case ID_IOCTL_POLL_EXIT: //23
			...
			elan_work_flag = TRUE_IRQ_NOTIFICATION;
		...
		case IOCTL_WRITE_KEY_STATUS:
			...
			key_status = arg;
			ELAN_DEBUG("IOCTL_WRITE_KEY_STATUS,key_status = %d\n", key_status);
			// guomingyi add.
...
//			}
        ...
        case ID_IOCTL_WRITE_FACTORY_STATUS:
            ...
            if (factory_status == FTM_START_STATUS)
            {
                elan_work_flag = TRUE_IRQ_NOTIFICATION;
                wake_up(&elan_poll_wq);
            }
...
static irqreturn_t efsa120s_irq_handler(int irq, void *_fp)
{
	...
	//}
	elan_work_flag = TRUE_IRQ_NOTIFICATION;
	if(factory_status == FTM_NORMAL_STATUS)
	    wake_up(&elan_poll_wq);
        else
            irq_status = 1;
	return IRQ_HANDLED;
...
//Add spi clk by yinglong.tang
#if BANPOD_READ_ID//!defined(CONFIG_MTK_CLKMGR)
...
static int get_finger_state(char *buf, void *args) 
{
    ELAN_DEBUG("efsa120s get_finger_state= %d\n",key_status);
...
{
    int ret = 0;
    ELAN_DEBUG("%s() Start, driver version = v830_1.2_factest\n", __func__);
