...
static char *fts_get_vendor_name(u8 vendor_id)
{
    switch(vendor_id)
    {
        ...
        case FTS_CTP_VENDOR_LIANCHUANG:
            return "LIANCHUANG";
...
//2.85V LDO always on, we need keep reset high to avoid current leak. Fred, 20190517
//        tpd_gpio_output(tpd_rst_gpio_number, 0);
        msleep(1);
...
//Fred add border touch for Plan B
//#define TP_BORDER_TOUCH_PIXELS 4
#ifdef TP_BORDER_TOUCH_PIXELS
int points_id_buff[FTS_MAX_POINTS_SUPPORT] = {0xff};
bool should_drop_point(int id)
{
	int i;
	for(i = 0; i < FTS_MAX_POINTS_SUPPORT; i++){
		if (id == points_id_buff[i]){
			return false;
		}
	}
	return true;
...
void store_points_id(int id)
{
	int i, temp = 0xff;
	bool found_first_empty_place = false;
	for(i = 0; i < FTS_MAX_POINTS_SUPPORT; i++){
		if (id == points_id_buff[i]){
			printk("[tinno tp border touch] points_id already exist\n");
			return;
		} else {
			if ((!found_first_empty_place) && (0xff == points_id_buff[i])) {
				temp = i;
	...
	if (temp >= FTS_MAX_POINTS_SUPPORT)
		printk("[tinno tp border touch] points_id_buff is full\n");
	else {
		points_id_buff[temp] = id;
		printk("[tinno tp border touch] id %d was stored in points_id_buff[%d]\n", id, temp);
	}
}

void reset_points_id(int id)
{
	int i;
	for(i = 0; i < FTS_MAX_POINTS_SUPPORT; i++){
		if (id == points_id_buff[i]){
			points_id_buff[i] = 0xff;
			return;
		}
	}
	printk("[tinno tp border touch] id %d is not in points_id_buff\n", id);
}

void clean_points_ids(void)
{
	int i;
	for(i = 0; i < FTS_MAX_POINTS_SUPPORT; i++)
		points_id_buff[i] = 0xff;

	printk("[tinno tp border touch] clean point id done\n");
}
#endif
/*****************************************************************************
...
#ifdef TP_BORDER_TOUCH_PIXELS
{
	int TINNO_TP_BORDER_TOUCH = TP_BORDER_TOUCH_PIXELS;    //tinnotp border touch function
	int tp_x_max = (int)TPD_RES_X - 1;

	/*************tinnotp border touch function****************/
	printk("[tinno tp border touch] P-ori (%d, %d), id = %d\n", events[i].x, events[i].y, events[i].id);
	if(TINNO_TP_BORDER_TOUCH > 0) {
		if(events[i].x > (tp_x_max - TINNO_TP_BORDER_TOUCH)) {
			if(events[i].x > (tp_x_max - TINNO_TP_BORDER_TOUCH/2)) {
				if (should_drop_point(events[i].id)){
					printk("[tinno tp border touch] drop point (%d, %d)\n", events[i].x, events[i].y);
					continue;
				}
			} else {
				events[i].x = events[i].x + events[i].x + TINNO_TP_BORDER_TOUCH - tp_x_max;
				printk("[tinno tp border touch] new point (%d, %d)\n", events[i].x, events[i].y);
			}
		}

		if((events[i].x < TINNO_TP_BORDER_TOUCH) && (events[i].y > 0)) {
			if(events[i].x < (TINNO_TP_BORDER_TOUCH/2)) {
				if (should_drop_point(events[i].id)){
					printk("[tinno tp border touch] drop point (%d, %d)\n", events[i].x, events[i].y);
					continue;
				}
			} else {
				events[i].x = events[i].x + events[i].x - TINNO_TP_BORDER_TOUCH;
				printk("[tinno tp border touch] new point (%d, %d)\n", events[i].x, events[i].y);
			}
		}
	}
...
#ifdef TEMP_DEBUG
    {
        int counter;
        int temp_points = 0;
        for(counter = 0; counter < max_touch_num; counter++) {
            if ((touchs >> counter)&0x01)
                temp_points++;
        }
        if(temp_points > max_touch_points)
            max_touch_points = temp_points;
    }
#endif
    if (va_reported) {
...
#ifdef TEMP_DEBUG
            first_touch = true;
            FTS_INFO("touch end, max_touch_points = %d", max_touch_points);
            max_touch_points = 0;
#endif
        } else {
...
//add for TinnoProductInfo
	...
	if (NORMAL_BOOT == get_boot_mode()){
		...
		if (ret) {
			...
			FTS_INFO("normal boot, enable fw auto update\n");
	}else{
		FTS_INFO("ftm recovery meta mode don't update tp \n");
	}
#endif

    tpd_load_status = 1;
    FTS_DEBUG("TPD_RES_Y:%d", (int)TPD_RES_Y);
    FTS_DEBUG("TPD_RES_X:%d", (int)TPD_RES_X);
#ifdef TP_BORDER_TOUCH_PIXELS
    clean_points_ids();
#endif
    FTS_FUNC_EXIT();
...
{
    int ret = 0;
    FTS_INFO("start fts_ftm_force_update\n");
