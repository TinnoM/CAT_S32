...
/*
 * fts_vendor_id interface
 */
#ifdef CONFIG_TINNO_PRODUCT_INFO
extern u8 global_vendor_id;
#endif
static ssize_t fts_vendor_id_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    int count = 0;
#ifdef CONFIG_TINNO_PRODUCT_INFO
    u8 local_vendor_id = global_vendor_id;
#else
    //temp value to avoid compiling errors
    u8 local_vendor_id = 0x87;
#endif
//    u8 val = 0;
//    struct i2c_client *client = container_of(dev, struct i2c_client, dev);
    struct input_dev *input_dev = fts_data->input_dev;
...
#endif
*/
    if (local_vendor_id == 0x87)
        count += snprintf(buf, PAGE_SIZE, "LianChuang");
    else if (local_vendor_id == 0x82)
        count += snprintf(buf, PAGE_SIZE, "HLT");
    else
        count += snprintf(buf, PAGE_SIZE, "Unknown");

    mutex_unlock(&input_dev->mutex);
    return count;
}

static ssize_t fts_vendor_id_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return -EPERM;
}
...
static DEVICE_ATTR(fts_vendor_id, S_IRUGO | S_IWUSR, fts_vendor_id_show, fts_vendor_id_store);
