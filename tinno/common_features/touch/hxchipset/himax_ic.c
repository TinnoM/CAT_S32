...
int himax_parse_criteria_file(void)
{
	...
	if (result == NULL) {
		E("%s,fail in line%d error code=%d\n", __func__, __LINE__, HX_INSPECT_EFILE);
		err = HX_INSPECT_EFILE;
		goto END_FUNC_REQ_RESULT_IDX_FAIL;
	}
	for (i = 0 ; i < data_size; i++) {
		...
		if (result[i] == NULL) {
			E("%s,fail in line%d error code=%d\n", __func__, __LINE__, HX_INSPECT_EFILE);
			err = HX_INSPECT_EFILE;
			goto END_FUNC_REQ_RESULT_ITEM_FAIL;
		}
	}
...
END_FUNC_REQ_RESULT_ITEM_FAIL:
	for (; i > 0; i--)
		kfree(result[i-1]);
	kfree(result);
END_FUNC_REQ_RESULT_IDX_FAIL:
...
int himax_self_test_data_init(void)
{
	...
	if (g_inspection_criteria == NULL) {
		...
		goto ALLOCATE_G_INSPEC_CRITRIA_IDX_FAILED;
	}
	for(i = 0;i < HX_CRITERIA_SIZE;i++) {
		...
		if (g_inspection_criteria[i] == NULL) {
			ret = HX_INSPECT_EOTHER;
			goto ALLOCATE_G_INSPEC_CRITRIA_ITEM_FAILED;
		}
	}

	return himax_parse_criteria_file();

ALLOCATE_G_INSPEC_CRITRIA_ITEM_FAILED:
	for (;i > 0; i--) {
		kfree(g_inspection_criteria[i - 1]);
		g_inspection_criteria[i - 1] = NULL;
	}
	kfree(g_inspection_criteria);
	g_inspection_criteria = NULL;
ALLOCATE_G_INSPEC_CRITRIA_IDX_FAILED:
	return ret;
}
...
void self_test_output(uint8_t *RB1H, uint16_t *mutual_bank, uint16_t *self_bank)
{
	...
	if (private_ts->suspended)
		fn = filp_open(g_sensor_result_file, O_APPEND|O_CREAT|O_RDWR, 0660);
	else
#endif
 ...
 /*   if (RB1H[0] == 0xAA)
	...
	if (RB1H[0] == 0xAA)
		...
		ret += snprintf(output_buffer + ret, len - ret, "Self_Test Fail\n");
#else
    if (RB1H[0] == 0xAA)
    {
        ret += snprintf(output_buffer + ret, len - ret, "\nSelf_Test Pass\n");
...
static uint8_t Self_Test_Bank(uint8_t *RB1H)
{
	...
	if (mutual_bank == NULL) {
		RB1H[0] = 0xF1;
		E("Allocating mutual_bank failed\n");
		goto ALLOCATING_MUTUAL_BANK_FAILED;
	}
    self_bank = kzalloc(self_num * sizeof(uint16_t), GFP_KERNEL);
	if (self_bank == NULL) {
		RB1H[0] = 0xF1;
		E("Allocating self_bank failed\n");
		goto ALLOCATING_SELF_BANK_FAILED;
	}
    himax_get_raw_data(g_diag_command, mutual_num, self_num); //get bank data
    if (RB1H[0] == 0x80)
    {
        I(" Enter Test flow \n");
		for (i = 0; i < mutual_num; i++) {
	...
	kfree(self_bank);
ALLOCATING_SELF_BANK_FAILED:
	kfree(mutual_bank);
ALLOCATING_MUTUAL_BANK_FAILED:
    return RB1H[0];
}
#endif
...
static void himax_set_gest_glove(struct i2c_client *client, uint8_t gest_en, uint8_t glv_en, bool suspended)
{
	uint8_t buf[4] = {0};
	uint8_t retry_cnt = 0;
	uint8_t back_data = 0;
	...
	buf[0] = (gest_en == 1 && suspended) ? 0x20 : 0x00;
#endif
#if defined(HX_HIGH_SENSE)
	buf[0] |= (glv_en == 1 && !suspended) ? 0x40 : 0x00;
#endif
	back_data = buf[0];
...
void himax_set_HSEN_enable(struct i2c_client *client, uint8_t HSEN_enable, bool suspended)
{
	uint8_t smwp_en = 0;
#if defined(HX_SMART_WAKEUP)
	smwp_en = private_ts->SMWP_enable;
#endif
	himax_set_gest_glove(client, smwp_en, HSEN_enable, suspended);
}
...
void himax_set_SMWP_enable(struct i2c_client *client, uint8_t SMWP_enable, bool suspended)
{
		uint8_t hsen_en = 0;
#if defined(HX_HIGH_SENSE)
	hsen_en = private_ts->HSEN_enable;
#endif

	himax_set_gest_glove(client, SMWP_enable, hsen_en, suspended);
}
...
#if defined(HX_SMART_WAKEUP)
	smwp_en = ts->SMWP_enable;
#endif
#if defined(HX_HIGH_SENSE)
	hesn_en = ts->HSEN_enable;
#endif

#if defined(HX_SMART_WAKEUP) || defined(HX_HIGH_SENSE)
	himax_set_gest_glove(ts->client, smwp_en, hesn_en, suspended);
#endif
#ifdef HX_USB_DETECT_GLOBAL
