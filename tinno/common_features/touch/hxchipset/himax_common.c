...
#if defined(HX_SMART_WAKEUP) && defined(HX_SELF_TEST_BLACK_SCREEN)
int suspend_resume_done = 0;
#endif
extern int g_diag_command;

#if defined(HX_AUTO_UPDATE_FW)
char i_CTPM_firmware_name[32] = "Himax_firmware.bin";
char *i_CTPM_FW = NULL;
unsigned int i_CTPM_FW_len;

#ifdef HX_AUTO_UPDATE_FW_COMPATIBLE_IFILE
unsigned char fw_i_arr[]= {
//#include "Himax_firmware.i"
#include "himax_v500_coe.i"
};
unsigned char *fw_buff_i = fw_i_arr;
int fw_size_i = sizeof(fw_i_arr);
#endif

unsigned int g_i_FW_VER = 0;
unsigned int g_i_CFG_VER = 0;
unsigned int g_i_CID_MAJ = 0; /*GUEST ID*/
unsigned int g_i_CID_MIN = 0; /*VER for GUEST*/
#endif
...
extern uint8_t *getFlashBuffer(void);
...
extern int32_t *getMutualNewBuffer(void);
extern int32_t *getMutualOldBuffer(void);
extern int32_t *getSelfBuffer(void);
...
extern bool Is_2T2R;
extern int32_t *getMutualBuffer_2(void);
...
//	if (i_CTPM_firmware_name == NULL) {
//		ret = MEM_ALLOC_FAIL;
//		goto ALLOCATE_FW_NAME_FAILED;
//	}
//	memcpy(i_CTPM_firmware_name, firmware_name, (sizeof(char)*(strlen(firmware_name)+1)));

//	I("%s, file name = %s\n", __func__, i_CTPM_firmware_name);
	ret_fw = request_firmware(&image, i_CTPM_firmware_name, private_ts->dev);
	if (ret_fw == 0) {
		I("%s: update firmware via bin, file name = %s\n", __func__, i_CTPM_firmware_name);
		...
		if (i_CTPM_FW != NULL) {
			...
			I("%s: i_CTPM_FW_len = %d ; i_CTPM_FW = %02X%02X%02X%02X\n", 
			__func__, i_CTPM_FW_len, i_CTPM_FW[0], i_CTPM_FW[1], i_CTPM_FW[2], i_CTPM_FW[3]);
		} else {
...
#ifdef HX_AUTO_UPDATE_FW_COMPATIBLE_IFILE
		I("%s: update firmware via i file\n", __func__);
		i_CTPM_FW_len = fw_size_i;
		i_CTPM_FW = kzalloc(sizeof(char)*i_CTPM_FW_len, GFP_KERNEL);
		if (i_CTPM_FW != NULL) {
			memcpy(i_CTPM_FW, fw_buff_i, sizeof(char)*i_CTPM_FW_len);
			I("%s: i_CTPM_FW_len = %d ; i_CTPM_FW = %02X%02X%02X%02X\n", 
			__func__, i_CTPM_FW_len, i_CTPM_FW[0], i_CTPM_FW[1], i_CTPM_FW[2], i_CTPM_FW[3]);
		} else {
			E("%s,%d: Allocate Memory Fail !!!!!\n", __func__, __LINE__);
...
static void himax_report_points(struct himax_ts_data *ts)
{
    ...
    else
    {
        ...
        for (loop_i = 0; loop_i < ts->nFinger_support; loop_i++)
        {
            /*if (((ts->pre_finger_mask >> loop_i) & 1) == 1)*/ //Do Not need to check pre_finger_mask
            {
#ifndef	HX_PROTOCOL_A
                input_mt_slot(ts->input_dev, loop_i);
				input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, 0);
...
int himax_checksum_cal(struct himax_ts_data *ts,uint8_t *buf,int *ts_status)
{
    ...
    if (*ts_status == HX_REPORT_COORD)
	{
		...
		if (private_ts->suspended 
			&& buf[GEST_PTLG_ID_LEN] == GEST_PTLG_HDR_ID1 && buf[GEST_PTLG_ID_LEN+1] == GEST_PTLG_HDR_ID2 
			&& ((buf[0] >= 0x01 && buf[0] <= 0x0F) || buf[0] == 0x80) 
			&& (buf[0] == buf[1] && buf[0] == buf[2] && buf[0] == buf[3]))
		{
			length = (GEST_PTLG_ID_LEN+GEST_PTLG_HDR_LEN);
			*ts_status = HX_REPORT_SMWP_EVENT;
			g_diag_command = 0;
		...
		if ((raw_data_sel != 0x0F) && (raw_data_sel != hx_touch_data->diag_cmd))/*raw data out not match skip it*/
			{
                ...
                if(!hx_touch_data->diag_cmd) {
                    himax_read_event_stack(ts->client, buf, (128-hx_touch_data->touch_info_size));/*Need to clear event stack here*/
...
//    if(himax_touch_get(ts,buf,ts_status))
    if(himax_touch_get(ts, ts->xfer_buff, ts_status))
        goto err_workqueue_out;
    ...
    //    himax_log_touch_data(buf,hx_touch_data);
        himax_log_touch_data(ts->xfer_buff, hx_touch_data);

    }

//    check_sum_cal = himax_checksum_cal(ts,buf,ts_status);
    check_sum_cal = himax_checksum_cal(ts, ts->xfer_buff, &ts_status);
    ...
    //    himax_assign_touch_data(buf,ts_status);
        himax_assign_touch_data(ts->xfer_buff, ts_status);
...
int himax_chip_common_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    setSysOperation(0);
    setFlashBuffer();
	if (getFlashBuffer() == NULL)
	{
		E("%s: Memory allocate fail !!!!!\n", __func__);
		err = -ENOMEM;
		goto err_allocate_flash_buffer_failed;
	}
#endif
...
FW_force_upgrade:
  ...
  queue_delayed_work(ts->himax_update_wq, &ts->work_update, msecs_to_jiffies(2000));
    ...
    setMutualNewBuffer();
    setMutualOldBuffer();
    if (getMutualBuffer() == NULL || getMutualNewBuffer() == NULL || getMutualOldBuffer() == NULL)
    {
...
#ifdef  HX_TP_PROC_FLASH_DUMP
	err_allocate_flash_buffer_failed:
...
END:
#if defined(HX_SMART_WAKEUP) && defined(HX_SELF_TEST_BLACK_SCREEN)
	if (g_self_test_entered)
		suspend_resume_done = 1;
#endif
    I("%s: END \n", __func__);
    return 0;
}
