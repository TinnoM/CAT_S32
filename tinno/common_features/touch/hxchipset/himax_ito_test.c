...
uint8_t himax_ito_test(void)
{
    ...
    {
	test_result = -ENOMEM;
        goto ALLOCATE_MUTUAL_BANK_FAILED;
    }
    ...
    {
	test_result = -ENOMEM;
        goto ALLOCATE_SELF_BANK_FAILED;
    }
...
ALLOCATE_ITO_LOG_BASEC_SELF_DATA_FAILED:
	kfree(hx_ito->ito_log_baseC_mul_data);
ALLOCATE_ITO_LOG_BASEC_MUL_DATA_FAILED:
	kfree(hx_ito->ito_log_self_data);
ALLOCATE_ITO_LOG_SELF_DATA_FAILED:
	kfree(hx_ito->ito_log_mul_data);
ALLOCATE_ITO_LOG_MUL_DATA_FAILED:
#endif
	kfree(hx_ito->noise_test_result);
ALLOCATE_NOISE_TEST_FAILED:
	kfree(hx_ito->self_rx_dev_result);
ALLOCATE_SELF_RX_DEV_FAILED:
	kfree(hx_ito->self_tx_dev_result);
ALLOCATE_SELF_TX_DEV_FAILED:
	kfree(hx_ito->mutual_rx_dev_result);
ALLOCATE_MUTUAL_RX_DEV_FAILED:
	kfree(hx_ito->mutual_tx_dev_result);
ALLOCATE_MUTUAL_TX_DEV_FAILED:
	kfree(hx_ito->self_baseC_percent_result);
ALLOCATE_SELF_BASEC_PERCENT_FAILED:
	kfree(hx_ito->mutual_baseC_percent_result);
ALLOCATE_MUTUAL_BASEC_PERCENT_FAILED:
	kfree(hx_ito->self_baseC);
ALLOCATE_SELF_BASEC_FAILED:
	kfree(hx_ito->mutual_baseC);
ALLOCATE_MUTUAL_BASEC_FAILED:
	kfree(hx_ito->self_iir_data);
ALLOCATE_SELF_IIR_DATA_FAILED:
	kfree(hx_ito->mutual_iir_data);
ALLOCATE_MUTUAL_IIR_DATA_FAILED:
	kfree(hx_ito->self_dc_data);
ALLOCATE_SELF_DC_DATA_FAILED:
	kfree(hx_ito->mutual_dc_data);
ALLOCATE_MUTUAL_DC_DATA_FAILED:
	kfree(hx_ito->self_bank_data);
ALLOCATE_SELF_BANK_DATA_FAILED:
	kfree(hx_ito->mutual_bank_data);
ALLOCATE_MULTUAL_BANK_DATA_FAILED:
    kfree(hx_ito);
ALLOCATE_HX_ITO_FAILED:
	kfree(self_bank);
ALLOCATE_SELF_BANK_FAILED:
	kfree(mutual_bank);
ALLOCATE_MUTUAL_BANK_FAILED:
