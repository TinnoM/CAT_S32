		...
		struct device_attribute *attr, const char *buf, size_t len)
{
	...
	if (!disksize)
       ...
       // [JSTINNO_SRC xiaoyan.yu,  add for zram size compatible, DATE20191122-01 START
       #if !defined(CONFIG_COMPAT) && defined(JOURNEY_FEATURE_ZRAM_SIZE_COMPATIBLE)
       {
            struct sysinfo si;
            si_meminfo(&si);
            if(si.totalram <= 786432 && si.totalram > 524288){ // 32bit 3GB RAM
                printk("Zram detect 3GB RAM");
                disksize = 805306368;  
            }
       }
       #endif/*CONFIG_COMPAT*/
       // JSTINNO_SRC xiaoyan.yu, DATE20191122-01 END]
