...
// pony.ma, DATE20191128, stop charging when reach set soc on demomode, DATE20191128-01 LINE
...
//			pr_info("Fred: charger plug in\n");
			cable_state = 1;
			mt_usb_connect();
		} else {
//			pr_info("Fred: charger plug out\n");
			cable_state = 0;
			mt_usb_disconnect();
		}
...
#if defined (CONFIG_TINNO_SCC_SUPPORT)
/* /sys/kernel/debug/scc/usb_online */
int scc_get_usb_online(void )
{
	return usb_online;
}

int scc_get_ac_online(void )
{	
	return ac_online;
}
EXPORT_SYMBOL(scc_get_capacity);
#endif  /* CONFIG_TINNO_SCC_SUPPORT */
...
static enum power_supply_property mt_charger_properties[] = {
	...
	// pony.ma, DATE20191128, add charge_enabled node, DATE20191128-01 START
	#ifdef CONFIG_TINNO_CHARGE_ENABLED_NODE
