...
#ifdef CONFIG_TINNO_CHARGE_ENABLED_NODE
static struct power_supply *chrdet_psy;
#endif  /* CONFIG_TINNO_CHARGE_ENABLED_NODE */

#if defined (CONFIG_TINNO_SCC_SUPPORT)
#include <linux/scc_drv.h>
#endif  /* CONFIG_TINNO_SCC_SUPPORT */

// pony.ma, DATE20191128, stop charging when reach set soc on demomode, DATE20191128-01 START
#ifdef CONFIG_TINNO_DEMOMODECHG_CONTROL
static struct power_supply *batdet_psy;
static int demomode_chg_max_soc = 100;
static int demomode_chg_min_soc = 0;
#endif  /* CONFIG_TINNO_DEMOMODECHG_CONTROL */
// pony.ma, DATE20191128-01 END
...
static void swchg_turn_on_charging(struct charger_manager *info)
{
	...
	// pony.ma, DATE20191128, add charge enable control node, DATE20191128-01 START
	#ifdef CONFIG_TINNO_CHARGE_ENABLED_NODE
	...
	if (!ret) {
		if(!flag1_adb){
			...
			if(info->demomode_enable){
				if(flag2_usb){				
					chr_err("ccc soc=%d,flag1_adb=%d,flag2_usb=%d,flag_soc=%d,max_soc=%d,min_soc=%d\n",
						soc,flag1_adb,flag2_usb,info->flag_soc,demomode_chg_max_soc,demomode_chg_min_soc);
					if (!((soc < demomode_chg_max_soc) && (info->flag_soc))){
						info->flag_soc = 0;
						charging_enable = false;
					...
					if((soc <= demomode_chg_min_soc) && (!info->flag_soc)){
						info->flag_soc = 1;
