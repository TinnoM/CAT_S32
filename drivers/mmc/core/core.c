...
extern unsigned int cd_gpio;  //tinno add for hotplug
static unsigned int card_slot_state=2;
extern void tinno_report_card_slot_state(unsigned char state);  //tinno add for hotplug
...
void mmc_rescan(struct work_struct *work)
{
	...
	cd_level = __gpio_get_value(cd_gpio);
	if(card_slot_state !=cd_level){
		tinno_report_card_slot_state(cd_level);
		card_slot_state =cd_level;
		printk("tinno test sd card:mmc_rescan  gpio:%d, level=%d\n",cd_gpio, cd_level);
	}
