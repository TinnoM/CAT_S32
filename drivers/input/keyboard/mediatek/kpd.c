...
{
	printk("Power Key generate, pressed=%ld\n", pressed);
	if (!kpd_input_dev) {
...
#define KEY_CARD_SLOT_INSERT 253
#define KEY_CARD_SLOT_REMOVE 254
void tinno_report_card_slot_state(unsigned char state)		//tinno add for hotplug
{
	if (!kpd_input_dev) {
		printk("KPD input device not ready for report card slot state\n");
		return;
	}
	if(state){
		input_report_key(kpd_input_dev, KEY_CARD_SLOT_INSERT, 1);
		input_report_key(kpd_input_dev, KEY_CARD_SLOT_INSERT, 0);
		input_sync(kpd_input_dev);
	}else
		{
		input_report_key(kpd_input_dev, KEY_CARD_SLOT_REMOVE, 1);
		input_report_key(kpd_input_dev, KEY_CARD_SLOT_REMOVE, 0);
		input_sync(kpd_input_dev);
	}
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	__set_bit(KEY_CARD_SLOT_INSERT, kpd_input_dev->keybit);     //tinno add for hotplug
	__set_bit(KEY_CARD_SLOT_REMOVE, kpd_input_dev->keybit);   //tinno add for hotplug
