...
int get_tp_fw_info(char *buf, void *arg0) 
{
	...
	if (idev->info_from_hex) {
		buf1[1] = idev->chip->info[44];
		buf1[2] = idev->chip->info[45];
		buf1[3] = idev->chip->info[46];
		buf1[4] = idev->chip->info[47];
	}
	...
	if (idev->read(buf1, idev->protocol->fw_ver_len) < 0) {
		ipio_err("i2c/spi read firmware ver err\n");
	}

	if (buf1[0] != P5_X_GET_FW_VERSION) {
		ipio_err("Invalid firmware ver\n");
	}

	ipio_info("Firmware version in ilitek_main.c = %d.%d.%d.%d\n", buf1[1], buf1[2], buf1[3], buf1[4]);
	idev->chip->fw_ver = buf1[1] << 24 | buf1[2] << 16 | buf1[3] << 8 | buf1[4];
...
//Tinno add start, Fred, 20191203
//clean point before enable irq to avoid ghost point issue
		ilitek_tddi_touch_release_all_point();
		//mdelay(20);
//Tinno add end, Fred, 20191203
