...
void ilitek_tddi_report_debug_mode(u8 *buf, int len)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		if (pressure == 1)
			...
			pressure = 1;
		touch_info[idev->finger].pressure = pressure;
		ipio_debug("original x = %d, y = %d\n", xop, yop);
		idev->finger++;
		if (MT_B_TYPE)
			idev->curt_touch[i] = 1;
...
void ilitek_tddi_report_gesture_mode(u8 *buf, int len)
{
	...
	case GESTURE_DOUBLECLICK:
		...
		gTGesture = 'u';
		input_report_key(idev->input, KEY_FTS_SENSOR, 1);
		input_sync(idev->input);
		input_report_key(idev->input, KEY_FTS_SENSOR, 0);
		input_sync(idev->input);
		break;
	case GESTURE_LEFT:
		break;
	...
	case GESTURE_DOWN:
		break;
	case GESTURE_O:
		ipio_info("KEY_O Click key event\n");
		gTGesture = 'o';
		input_report_key(idev->input, KEY_FTS_SENSOR, 1);
		input_sync(idev->input);
		input_report_key(idev->input, KEY_FTS_SENSOR, 0);
	...
	case GESTURE_W:
		break;
	case GESTURE_M:
		ipio_info("KEY_M Click key event\n");
		gTGesture = 'm';
		input_report_key(idev->input, KEY_FTS_SENSOR, 1);
		input_sync(idev->input);
		input_report_key(idev->input, KEY_FTS_SENSOR, 0);
	...
	case GESTURE_Z:
		break;
	case GESTURE_C:
		ipio_info("KEY_C Click key event\n");
		gTGesture = 'c';
		input_report_key(idev->input, KEY_FTS_SENSOR, 1);
		input_sync(idev->input);
		input_report_key(idev->input, KEY_FTS_SENSOR, 0);
