...
void ilitek_plat_input_register(void)
{
	...
	input_set_capability(idev->input, EV_KEY, 249);
	...
	__set_bit(249,	idev->input->keybit);
	__set_bit(KEY_POWER,  idev->input->keybit);
}
...
int ilitek_plat_irq_register(int type)
{
	...
	ret = devm_request_threaded_irq(idev->dev, idev->irq_num,
	    ...
	    //ret = request_irq(idev->irq_num, ilitek_plat_isr_bottom_half,
        //              IRQF_TRIGGER_RISING, "ilitek", NULL);
