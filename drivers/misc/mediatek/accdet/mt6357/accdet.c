...
#ifdef CONFIG_WATER_IN_DETECT
/* hp_eint_timer: detect the hp_eint pin's voltage
 * timeout: 3 seconds
 * timerHandler: hp_eint_timerhandler()
 */
#define HP_EINT_TIMEOUT (2 * HZ)
static struct timer_list hp_eint_timer;
static void hp_eint_timerhandler(unsigned long data);
/* Default channel of hp_eint adc is 3 */
static int hp_eint_adc_channel;
/* Default voltage division ratio is 2 */
static int vol_div_ratio;
/* Default first timeout of hp_eint_timer is 750ms */
static int start_time;
static unsigned int get_hp_eint_vol(void);
static struct work_struct hp_eint_timer_work;
#define EINT_VOL_THR_H 2000	//20000mV
#define EINT_VOL_THR_L 560	//560mV, 25% of EINT_VOL_H
#define EINT_VOL_H 2240
#define REG_EINT_HIGH_R_500K (1 << 12)
#define REG_EINTCOMPVTH_2V (1 << 10)
#endif
...
#ifdef CONFIG_WATER_IN_DETECT
static ssize_t show_hp_eint_vol(struct device_driver *ddri, char *buf)
{
	if (buf == NULL) {
		pr_notice("[%s] *buf is NULL!\n",  __func__);
		return -EINVAL;
...
static DRIVER_ATTR(hp_eint_vol, 0644, show_hp_eint_vol, NULL);
#endif
...
static unsigned int get_hp_eint_vol(void)
{
	int ret, raw_data, data[4];

	ret = IMM_GetOneChannelValue(hp_eint_adc_channel, data, &raw_data);
	if (ret) {
		pr_err("%s: get hp_eint voltage error\n", __func__);
		return EINT_VOL_H;
	}

	return (data[0] * 1000 + data[2]) * vol_div_ratio;
}

static int pmic_eint_queue_work(int eintID);
static void hp_timer_wc(struct work_struct *work)
{
	unsigned int hp_eint_vol = get_hp_eint_vol();
	pr_debug("%s: hp_eint_vol is %d\n", __func__, hp_eint_vol);

	if (hp_eint_vol >= EINT_VOL_THR_L) {
		switch (cable_type) {
		case HEADSET_MIC:
		case HEADSET_NO_MIC:
			pr_debug("%s : cable plugout\n", __func__);
			/* clc Accdet PWM idle */
			pmic_write(ACCDET_STATE_SWCTRL,
		...
		case NO_DEVICE:
		default:
			pr_debug("%s: cable pluged out already, do nothing\n", __func__);
			break;
		}
	} else {
		switch (cable_type) {
		case HEADSET_MIC:
		case HEADSET_NO_MIC:
			pr_debug("%s: cable pluged in already, do nothing\n", __func__);
			break;
		case NO_DEVICE:
		default:
			//do plug_in progress.
	...
	if (hp_eint_vol <= EINT_VOL_THR_H) {
		pr_debug("%s: modify timer\n", __func__);
		mod_timer(&hp_eint_timer, jiffies + HP_EINT_TIMEOUT);
	}
...
{
	pr_debug("%s\n", __func__);
	schedule_work(&hp_eint_timer_work);
	return;
}
#endif
...
#ifdef CONFIG_WATER_IN_DETECT
	hp_eint_vol = get_hp_eint_vol();
	pr_debug("%s: hp_eint_vol = %d\n", __func__, hp_eint_vol);
	if (hp_eint_vol <= EINT_VOL_THR_H && hp_eint_vol > EINT_VOL_THR_L) {
		mutex_lock(&accdet_eint_irq_sync_mutex);
		pr_info("%s: Maybe water in, set accdet_status to PLUG_OUT\n",
			__func__);
		accdet_status = PLUG_OUT;
		mod_timer(&hp_eint_timer, jiffies + msecs_to_jiffies(start_time));
		mutex_unlock(&accdet_eint_irq_sync_mutex);
		return;
	}
#endif
...
#ifdef CONFIG_WATER_IN_DETECT
	if (accdet_status == MIC_BIAS || accdet_status == HOOK_SWITCH) {
		pr_info("%s: cable in, modify hp_eint_timer\n", __func__);
		mod_timer(&hp_eint_timer, jiffies + HP_EINT_TIMEOUT);
	}
#endif
...
#ifdef CONFIG_WATER_IN_DETECT
	if (of_property_read_u32(node, "hp-eint-adc-channel",
			&hp_eint_adc_channel)) {
		pr_err("%s: get hp-eint-adc-channel err, set default value!\n",
				__func__);
		hp_eint_adc_channel = 3;
	}

	if (of_property_read_u32(node, "hp-vol-div-ratio",
			&vol_div_ratio)) {
		pr_err("%s: get vol-div-ratio error, set default value!\n",
				__func__);
		vol_div_ratio = 2;
	}

	if (of_property_read_u32(node, "hp-start-time",
			&start_time)) {
		pr_err("%s: get start-time error, set default value!\n",
				__func__);
		start_time = 750;
	}
#endif
...
#ifdef CONFIG_WATER_IN_DETECT
	/* set EINT HIGH R to 500k */
	reg |= REG_EINT_HIGH_R_500K;
	/* set EINT comparator threshold to 2V*/
	reg |= REG_EINTCOMPVTH_2V;
#endif
	/* ACC mode*/
...
#ifdef CONFIG_WATER_IN_DETECT
	/* init the timer for hp_eint pin voltage detect*/
	init_timer(&hp_eint_timer);
	hp_eint_timer.expires = jiffies + HP_EINT_TIMEOUT;
	hp_eint_timer.function = &hp_eint_timerhandler;
	hp_eint_timer.data = 0;

	INIT_WORK(&hp_eint_timer_work, hp_timer_wc);
#endif
