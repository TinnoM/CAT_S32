...
// [JSTINNO_SRC xiaoyan.yu,  add for tracking high cpu loading info, -01 START
void print_cpu_loading_info(int tmp_cpu_loading){
	char cpu_info[250] = {0};
	char temp[20] = {0};
	int i ;
	for_each_possible_cpu(i) {
	    sprintf(temp, "CPU%d:%d",i, cpufreq_get(i));
	    strncat(cpu_info, temp, strlen(temp));
	}
	printk("High CPU Loading warnning : cpu_loading:%d prev_cpu_loading:%d , %s\n",
			tmp_cpu_loading, prev_cpu_loading,cpu_info);
}
...
int update_cpu_loading(void)
{
	...
	if (state == high) {
		if (under_threshold > tmp_cpu_loading) {
			...
			#ifdef JOURNEY_FEATURE_TRACK_CPU_HIGH_LOADING
			print_cpu_loading_info(prev_cpu_loading);
			#endif/*JOURNEY_FEATURE_TRACK_CPU_HIGH_LOADING*/
