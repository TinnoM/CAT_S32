 ...
 * Filename:
 * ---------
 *	 s5k3l6_tsp_v500_mipiraw_Sensor.c
 *
 * Project:
 * --------
 *	 ALPS
...
#define PFX "S5K3l6_tsp_camera_sensor"
...
#include "s5k3l6_tsp_v500_mipiraw_Sensor.h"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K3L6_TSP_V500_SENSOR_ID,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_info("s5k3l6 tsp read sensor id: 0x%x module_id: 0x%x\n", *sensor_id, module_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				//tsp module_id no equal 0x06
				if(module_id != 0x06) {
					pr_info("s5k3l6 tsp module id matched!\n");
					return ERROR_NONE;
				} else {
					pr_info("s5k3l6 tsp module not matched, return error!\n");
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			pr_info("s5k3l6 tsp i2c write id: 0x%x, sensor id: 0x%x module_id 0x%x\n", imgsensor.i2c_write_id,sensor_id, module_id);
			if (sensor_id == imgsensor_info.sensor_id) {
					pr_info("s5k3l6 get sunwin module_id  matched\n");
					break;
			}
			pr_info("s5k3l6 tsp Read sensor id fail: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
...
UINT32 S5K3L6_TSP_V500_MIPI_RAW_SensorInit(
